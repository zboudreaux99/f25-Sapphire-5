...
function PMTenants({ show, handleClose, propertyId = 1, onDataChanged }) {
  ...
  const handleSaveNewTenant = async () => {
    ...
    try {
      const res = await fetch(
        "http://localhost:8080/api/property/unit/tenant",
        {
          method: "POST",
  ...
  const handleRemoveTenant = async (tenantId) => {
    try {
      const res = await fetch(
        "http://localhost:8080/api/property/unit/tenant/remove",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        ...
        <Accordion>
          {tenants.map((tenant, idx) => (
            <Accordion.Item eventKey={idx.toString()} key={tenant.tenant_id}>
              ...
              <Accordion.Body>
                <p>
                  <strong>Unit:</strong> {tenant.unit_name}
                </p>
                <p>
                  <strong>Property:</strong> {tenant.property_name}
                </p>
                <p>
                  <strong>Complaints:</strong> {tenant.complaints_initiated}
                </p>
