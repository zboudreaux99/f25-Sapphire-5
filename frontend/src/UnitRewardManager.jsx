...
function UnitRewardManager({ propertyId = 1 }) {
  ...
  const fetchRewards = async () => {
    try {
      const res = await fetch(
        `http://localhost:8080/api/property/rewards?property_id=${propertyId}`
      );
  ...
  const fetchUnitRewards = async () => {
    try {
      ...
      for (const unit of units) {
        const res = await fetch(
          `http://localhost:8080/api/property/unit/rewards?unit_id=${unit.unit_id}`
        );
        const data = await res.json();
        if (Array.isArray(data)) {
          allRewards.push(
            ...data.map((r) => ({ ...r, unit_id: unit.unit_id }))
          );
        }
      }
  ...
  const handleAssignReward = async () => {
    ...
    try {
      const res = await fetch(
        `http://localhost:8080/api/property/unit/rewards`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
              ...
              return (
                <div key={unit.unit_id}>
                  ...
                  <Collapse in={openUnits[unit.unit_id]}>
                    <div
                      id={`unit-collapse-${unit.unit_id}`}
                      className="ps-3 pt-1 pb-2 text-white"
                    >
                      {assigned}
                    </div>
                  </Collapse>
